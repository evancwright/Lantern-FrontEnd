    ...
    partial class VMBase
    {
        ...
        public void Fetch()
        {
            ...
            {//is a prefix
                ...
                ir[1] = GetByte(pc);
                opcode = (ushort)(ir[0] * 256 + ir[1]);

                AdvancePC();

                //how do we figure out if there's 0,1,or 2 immediates?
                if (PrefixedWith1Immediate())
                {
                    ir[2] = GetByte(pc);
                    AdvancePC();
                }
                else if (PrefixedWith2Immediates())
                {
                    ir[2] = GetByte(pc);
                    AdvancePC();
                ...
                else if (PrefixedWithImmediateWord())
                {
                    ir[2] = GetByte(pc);
                    AdvancePC();
        ...
        //for 1 byte op codes
        bool HasImmediateByte(byte x, byte y, byte z)
        {
            if (x == 0)
            {
                if (z == 0 && y >= 2 && y <= 7)
        ...
        bool HasImmediateWord(byte p, byte q, byte x, byte y, byte z)
        {
            if (x == 0)
            {
                if (z == 1 && q == 0)
        ...
        bool PrefixedWithImmediateWord()
        {
            if (ir[0] == 0xFD || ir[0] == 0xDD)
            {
                if (ir[1] == 0x22 || 
